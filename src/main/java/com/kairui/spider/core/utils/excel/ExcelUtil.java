...
public class ExcelUtil<T> {
    ...
    public List<T> importExcel(Boolean xlsx, String sheetName, InputStream input) {
        ...
        try {
            Workbook workbook;
            if (xlsx) workbook = new XSSFWorkbook(OPCPackage.open(input));
            else workbook = new HSSFWorkbook(input);
            Sheet sheet;
            if (sheetName != null && !sheetName.trim().equals("")) sheet = workbook.getSheet(sheetName);//如果指定sheet名,则取指定sheet中的内容.
